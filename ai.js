...
function AITest(p) {
  ...
  // Изменённое имя AI для русскоязычного игрока.
  p.name = "ИИ Тест " + this.constructor.count;

  // Решаем, покупать ли собственность, на которую попал AI.
  // Return: boolean (true to buy).
  // Arguments:
  // index: the property's index (0-39).
  this.buyProperty = function (index) {
    console.log("купитьСвойство");
    var s = square[index];
  ...
  // Определяет ответ на предложенную сделку.
  ...
  // Вызывается в начале хода AI до броска кубиков.
  ...
  this.beforeTurn = function () {
    ...
    for (var i = 0; i < 40; i++) {
      ...
      if (s.owner === p.index && s.groupNumber >= 3) {
        max = s.group.length;
        allGroupOwned = true;
        leastHouseNumber = 6; // Ни у одной собственности не может быть 6 домов.
  ...
  // Вызывается каждый раз, когда AI выходит на клетку.
  // Return: boolean: Must return true if and only if the AI proposed a trade.
  this.onLand = function () {
    console.log("наКлетке");
    var proposedTrade;
    var property = [
    ...
    // Если AI владеет ровно одной коммунальной службой, пробуем обменять её на железную дорогу.
    for (var i = 0; i < 4; i++) {
    ...
    if (
      utilityForRailroadFlag &&
      game.getDie(1) !== game.getDie(2) &&
      requestedRailroad &&
      offeredUtility
    ) {
      ...
      proposedTrade = new Trade(
        p,
        player[square[requestedRailroad].owner],
        0,
        property,
        0,
        0
  ...
  // Определяет, следует ли AI заплатить залог или использовать карту выхода из тюрьмы.
  // Return: boolean: true to post bail/use card.
  this.postBail = function () {
    console.log("оплатаЗалогов");
  ...
  // Определяет, какую ставку сделать на аукционе.
