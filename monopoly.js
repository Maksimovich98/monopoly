...
function Game() {
    ...
    if (!p.human && p.money < 0) {
      ...
      if (p.money < 0) {
        popup(
          "<p>" +
            p.name +
            " обанкротился. Все его активы будут переданы " +
  ...
  this.auction = function () {
    ...
    popup(
      "<div style='font-weight: bold; font-size: 16px; margin-bottom: 10px;'>Аукцион <span id='propertyname'></span></div><div>Макс. ставка = $<span id='highestbid'></span> (<span id='highestbidder'></span>)</div><div><span id='currentbidder'></span>, теперь ваша очередь делать ставку.</div><div><input id='bid' title='Введите сумму для ставки на " +
        s.name +
        ".' style='width: 291px;' /></div><div><input type='button' value='Ставка' onclick='game.auctionBid();' title='Сделать ставку.' /><input type='button' value='Пас' title='Пропустить ход для ставок.' onclick='game.auctionPass();' /><input type='button' value='Выйти из аукциона' title='Полностью отказаться от участия в аукционе по " +
        s.name +
        ".' onclick='if (confirm(\"Вы уверены, что хотите выйти из аукциона по этому свойству?\")) game.auctionExit();' /></div>",
      "blank"
    );
      ...
      if (currentbidder == highestbidder) {
        ...
        if (!p.human) {
          ...
          if (bid === -1 || highestbid >= p.money) {
            ...
            window.alert(p.name + " пропустил ход.");
            continue;
          } else if (bid > 0) {
            this.auctionBid(bid);
            window.alert(p.name + " сделал ставку $" + bid + ".");
            continue;
          }
          return;
        } else {
  ...
  this.auctionBid = function (bid) {
    ...
    if (bid === "" || bid === null) {
      ...
      if (bid > player[currentbidder].money) {
        document.getElementById("bid").value =
          "У вас недостаточно средств для ставки $" + bid + ".";
        document.getElementById("bid").style.color = "red";
        ...
        document.getElementById("bid").value =
          "Ваша ставка должна быть больше максимальной ставки ($" +
          highestbid +
          ").";
        document.getElementById("bid").style.color = "red";
      }
  ...
  var resetTrade = function (initiator, recipient, allowRecipientToBeChanged) {
    ...
    for (var i = 0; i < 40; i++) {
      ...
      if (currentSquare.owner === initiator.index) {
        ...
        currentTableCellCheckbox.title =
          "Отметьте этот флажок, чтобы включить " +
          currentSquare.name +
          " в обмен.";
        ...
        if (currentSquare.mortgage) {
          currentTableCell.title = "Заложено";
    ...
    if (initiator.communityChestJailCard) {
      ...
      currentTableCellCheckbox.title =
        'Отметьте этот флажок, чтобы включить эту карту "Выход из тюрьмы" в обмен.';
      ...
      currentTableCell.textContent = 'Карта "Выход из тюрьмы"';
    ...
    if (initiatorSideTable.lastChild) {
      ...
      initiatorProperty.textContent =
        initiator.name + " не имеет свойств для обмена.";
    }
    ...
    if (recipientSideTable.lastChild) {
      ...
      recipientProperty.textContent =
        recipient.name + " не имеет свойств для обмена.";
    }
    ...
    if (allowRecipientToBeChanged && pcount > 2) {
      ...
      nameSelect.title = "Выберите игрока для обмена.";
    } else {
  ...
  this.acceptTrade = function (tradeObj) {
    ...
    if (isAPropertySelected === 0) {
      popup("<p>Для обмена необходимо выбрать хотя бы одно свойство.</p>");
    ...
    if (
      ...
      !confirm(
        initiator.name +
          ", вы уверены, что хотите сделать этот обмен с " +
          recipient.name +
          "?"
      )
    ) {
  ...
  this.proposeTrade = function () {
    ...
    var initiator = tradeObj.getInitiator(); // corrected variable name
    ...
    if (
      ...
      !confirm(
        initiator.name +
          ", вы уверены, что хотите предложить обмен с " +
          recipient.name +
          "?"
      )
    ) {
  ...
  this.eliminatePlayer = function () {
    ...
    if (pcount === 1) {
      ...
      popup(
        "<p>Поздравляем, " + player[1].name + ", вы выиграли игру.</p><div>"
      );
    } else {
      play();
    }
  ...
  this.bankruptcyUnmortgage = function () {
    ...
    var HTML =
      "<p>" +
      player[p.creditor].name +
      ", вы можете выкупить любое из следующих свойств без процентов, нажав на них. Нажмите ОК, когда закончите.</p><table>";
    var price;
    ...
    for (var i = 0; i < 40; i++) {
      ...
      if (sq.owner == p.index && sq.mortgage) {
        ...
        HTML +=
          ...
          i +
          ");' onmouseout='hidedeed();'></td><td class='propertycellname'><a href='javascript:void(0);' title='Выкупить " +
          sq.name +
          " за $" +
          price +
          ".' onclick='if (" +
          ...
          " за $" +
          price +
          '.");} this.parentElement.parentElement.style.display = "none";\'>Выкупить ' +
          sq.name +
          " ($" +
          price +
          ")</a></td></tr>";
    ...
    if (pcount === 2 || bankruptcyUnmortgageFee === 0 || p.creditor === 0) {
      ...
      addAlert(
        pcredit.name +
          ...
          " процентов за заложенные свойства, полученные от " +
          p.name +
          "."
      );
      popup(
...
function updateOwned() {
  ...
  for (var i = 0; i < 40; i++) {
    ...
    if (sq.owner == turn) {
      mortgagetext = "";
      if (sq.mortgage) {
        mortgagetext = "title='Заложено' style='color: grey;'";
      }
      ...
      if (sq.house >= 1 && sq.house <= 4) {
        for (var x = 1; x <= sq.house; x++) {
          housetext +=
            "<img src='images/house.png' alt='' title='Дом' class='house' />";
        }
      } else if (sq.hotel) {
        housetext +=
          "<img src='images/hotel.png' alt='' title='Отель' class='hotel' />";
      }
  ...
  if (p.communityChestJailCard) {
    ...
    HTML +=
      "<tr class='property-cell-row'><td class='propertycellcheckbox'><input type='checkbox' id='propertycheckbox40' /></td><td class='propertycellcolor' style='background: white;'></td><td class='propertycellname'>Карта \"Выход из тюрьмы\"</td></tr>";
  }
  if (p.chanceJailCard) {
    if (HTML === "") {
      firstproperty = 41;
    ...
    HTML +=
      "<tr class='property-cell-row'><td class='propertycellcheckbox'><input type='checkbox' id='propertycheckbox41' /></td><td class='propertycellcolor' style='background: white;'></td><td class='propertycellname'>Карта \"Выход из тюрьмы\"</td></tr>";
  }

  if (HTML === "") {
    HTML = p.name + ", у вас нет свойств.";
    $("#option").hide();
  } else {
    HTML += "</table>";
  }
...
function updateOption() {
  ...
  if (sq.mortgage) {
    document.getElementById("mortgagebutton").value =
      "Выкупить ($" + Math.round(sq.price * 0.6) + ")";
    document.getElementById("mortgagebutton").title =
      "Выкупить " + sq.name + " за $" + Math.round(sq.price * 0.6) + ".";
    $("#buyhousebutton").hide();
    $("#sellhousebutton").hide();
    ...
    document.getElementById("mortgagebutton").value =
      "Заложить ($" + sq.price * 0.5 + ")";
    document.getElementById("mortgagebutton").title =
      "Заложить " + sq.name + " за $" + sq.price * 0.5 + ".";
    ...
    if (sq.groupNumber >= 3) {
      ...
      if (sq.hotel == 1) {
        $("#buyhousebutton").hide();
        sellhousebutton.value = "Продать отель ($" + sq.houseprice * 0.5 + ")";
        sellhousebutton.title = "Продать отель за $" + sq.houseprice * 0.5;
      }
      ...
      for (var i = 0; i < max; i++) {
        ...
        if (s.owner !== sq.owner) {
          ...
          buyhousebutton.title =
            "Прежде чем купить дом, вы должны владеть всеми свойствами этой цветовой группы.";
        } else {
          if (s.house > maxhouse) {
            maxhouse = s.house;
          }
      ...
      if (!allGroupUnmortgaged) {
        buyhousebutton.disabled = true;
        buyhousebutton.title =
          "Прежде чем купить дом, вы должны выкупить все свойства этой цветовой группы.";
      }
      ...
      if (sq.house > minhouse) {
        ...
        if (sq.house == 1) {
          buyhousebutton.title =
            "Прежде чем купить еще один дом, другие свойства этой цветовой группы должны иметь по одному дому.";
        } else if (sq.house == 4) {
          buyhousebutton.title =
            "Прежде чем купить отель, другие свойства этой цветовой группы должны иметь по 4 дома.";
        } else {
          buyhousebutton.title =
            "Прежде чем купить дом, другие свойства этой цветовой группы должны иметь " +
            sq.house +
            " домов.";
        }
      }
      if (sq.house < maxhouse) {
        ...
        if (sq.house == 1) {
          sellhousebutton.title =
            "Прежде чем продать дом, другие свойства этой цветовой группы должны иметь по одному дому.";
        } else {
          sellhousebutton.title =
            "Прежде чем продать дом, другие свойства этой цветовой группы должны иметь " +
            sq.house +
            " домов.";
        }
      }
      ...
      if (!allGroupUninproved) {
        document.getElementById("mortgagebutton").title =
          "Прежде чем заложить свойство, все свойства этой цветовой группы должны быть неулучшенными.";
...
function chanceCommunityChest() {
  ...
  if (p.position === 2 || p.position === 17 || p.position === 33) {
    ...
    popup(
      "<img src='images/community_chest_icon.png' style='height: 50px; width: 53px; float: left; margin: 8px 8px 8px 0px;' /><div style='font-weight: bold; font-size: 16px; '>Общественная казна:</div><div style='text-align: justify;'>" +
    ...
    popup(
      "<img src='images/chance_icon.png' style='height: 50px; width: 26px; float: left; margin: 8px 8px 8px 0px;' /><div style='font-weight: bold; font-size: 16px; '>Шанс:</div><div style='text-align: justify;'>" +
...
function addamount(amount, cause) {
  ...
  addAlert(p.name + " получил $" + amount + " от " + cause + ".");
}
...
function subtractamount(amount, cause) {
  ...
  addAlert(p.name + " потерял $" + amount + " от " + cause + ".");
}

function gotojail() {
  var p = player[turn];
  addAlert(p.name + " отправлен в тюрьму.");
  ...
  document.getElementById("nextbutton").title =
    "Закончить ход и передать ход следующему игроку.";
...
function collectfromeachplayer(amount, cause) {
  ...
  addAlert(p.name + " получил $" + total + " от " + cause + ".");
}
...
function advance(destination, pass) {
  ...
  if (typeof pass === "number") {
    if (p.position < pass) {
      ...
      p.position = pass;
      p.money += 200;
      addAlert(p.name + " получил $200 за прохождение через GO.");
    }
  }
...
function streetrepairs(houseprice, hotelprice) {
  ...
  if (cost > 0) {
    ...
    if (houseprice === 40) {
      addAlert(p.name + " потерял $" + cost + " в Общественную казну.");
    } else {
      addAlert(p.name + " потерял $" + cost + " в Шанс.");
    }
  }
}
...
function payfifty() {
  ...
  addAlert(p.name + " заплатил штраф $50, чтобы выйти из тюрьмы.");
  updateMoney();
  updatePosition();
}
...
function useJailCard() {
  ...
  addAlert(p.name + ' использовал карту "Выход из тюрьмы".');
  updateOwned();
  updatePosition();
}
  ...
  if (p.money - sq.houseprice < 0) {
    ...
    if (sq.house < 4) {
      if (houseSum >= 32) {
        ...
        sq.house++;
        addAlert(p.name + " построил дом на " + sq.name + ".");
      }
    } else {
      if (hotelSum >= 12) {
        return;
        ...
        sq.house = 5;
        sq.hotel = 1;
        addAlert(p.name + " построил отель на " + sq.name + ".");
      }
    }
...
function sellHouse(index) {
  ...
  if (sq.hotel === 1) {
    sq.hotel = 0;
    sq.house = 4;
    addAlert(p.name + " продал отель на " + sq.name + ".");
  } else {
    sq.house--;
    addAlert(p.name + " продал дом на " + sq.name + ".");
  }
  ...
  for (var x = 1; x <= pcount; x++) {
    ...
    for (var i = 0; i < 40; i++) {
      ...
      if (sq.owner == x) {
        ...
        if (sq.house == 5) {
          housetext +=
            "<span style='float: right; font-weight: bold;'>1&nbsp;x&nbsp;<img src='images/hotel.png' alt='' title='Отель' class='hotel' style='float: none;' /></span>";
        } else if (sq.house > 0 && sq.house < 5) {
          housetext +=
            "<span style='float: right; font-weight: bold;'>" +
            sq.house +
            "&nbsp;x&nbsp;<img src='images/house.png' alt='' title='Дом' class='house' style='float: none;' /></span>";
        }
    ...
    if (p.communityChestJailCard) {
      ...
      HTML +=
        "<tr><td class='statscellcolor'></td><td class='statscellname'>Карта \"Выход из тюрьмы\"</td></tr>";
    }
    if (p.chanceJailCard) {
      if (!write) {
        write = true;
...
function buy() {
  ...
  if (p.money >= cost) {
    ...
    updateMoney();
    addAlert(
      p.name + " купил " + property.name + " за " + property.pricetext + "."
    );
...
function mortgage(index) {
  ...
  document.getElementById("mortgagebutton").value =
    "Выкупить за $" + unmortgagePrice;
  document.getElementById("mortgagebutton").title =
    "Выкупить " + sq.name + " за $" + unmortgagePrice + ".";

  addAlert(p.name + " заложил " + sq.name + " за $" + mortgagePrice + ".");
  updateOwned();
  updateMoney();
...
function unmortgage(index) {
  ...
  sq.mortgage = false;
  document.getElementById("mortgagebutton").value =
    "Заложить за $" + mortgagePrice;
  document.getElementById("mortgagebutton").title =
    "Заложить " + sq.name + " за $" + mortgagePrice + ".";

  addAlert(p.name + " выкупил " + sq.name + " за $" + unmortgagePrice + ".");
  updateOwned();
  return true;
}
...
function land(increasedRent) {
  ...
  document.getElementById("landed").innerHTML =
    "Вы приземлились на " + s.name + ".";
  s.landcount++;
  addAlert(p.name + " приземлился на " + s.name + ".");
  ...
  if (s.price !== 0 && s.owner === 0) {
    if (!p.human) {
      ...
      document.getElementById("landed").innerHTML =
        "<div>Вы приземлились на <a href='javascript:void(0);' onmouseover='showdeed(" +
        p.position +
        ");' onmouseout='hidedeed();' class='statscellcolor'>" +
        s.name +
        "</a>.<input type='button' onclick='buy();' value='Купить ($" +
        s.price +
        ")' title='Купить " +
        s.name +
        " за " +
  ...
  if (s.owner !== 0 && s.owner != turn && !s.mortgage) {
    ...
    addAlert(
      p.name + " заплатил $" + rent + " аренды " + player[s.owner].name + "."
    );
    p.pay(rent, s.owner);
    ...
    document.getElementById("landed").innerHTML =
      "Вы приземлились на " +
      s.name +
      ". Свойство заложено; аренда не взималась.";
  }
  ...
  if (p.position === 30) {
    ...
    if (p.human) {
      popup(
        "<div>Идите в тюрьму. Идите прямо в тюрьму. Не проходите через GO. Не получайте $200.</div>",
        goToJail
      );
    } else {
      gotojail();
...
function roll() {
  ...
  document.getElementById("nextbutton").title =
    "Бросьте кубики и переместите свою фишку соответственно.";
  ...
  if (die1 == die2) {
    addAlert(p.name + " выбросил " + (die1 + die2) + " - дубль.");
  } else {
    addAlert(p.name + " выбросил " + (die1 + die2) + ".");
  }
  ...
  if (die1 == die2 && !p.jail) {
    ...
    if (doublecount < 3) {
      ...
      document.getElementById("nextbutton").title =
        "Вы выбросили дубль. Бросьте снова.";

      // If player rolls doubles three times in a row, send him to jail
    } else if (doublecount === 3) {
      p.jail = true;
      doublecount = 0;
      addAlert(p.name + " выбросил дубль три раза подряд.");
      updateMoney();

      if (p.human) {
        popup("Вы выбросили дубль три раза подряд. Идите в тюрьму.", goToJail);
      } else {
        gotojail();
      }
  ...
  if (p.jail === true) {
    ...
    if (die1 == die2) {
      ...
      addAlert(p.name + " выбросил дубль, чтобы выйти из тюрьмы.");

      land();
    } else {
      if (p.jailroll === 3) {
        if (p.human) {
          popup("<p>Вы должны заплатить штраф $50.</p>", function () {
            payFifty();
            payfifty();
...
function play() {
  ...
  if (p.jail) {
    $("#landed").show();
    document.getElementById("landed").innerHTML =
      "Вы в тюрьме.<input type='button' title='Заплатить штраф $50, чтобы выйти из тюрьмы немедленно.' value='Заплатить штраф $50' onclick='payfifty();' />";

    if (p.communityChestJailCard || p.chanceJailCard) {
      document.getElementById("landed").innerHTML +=
        "<input type='button' id='gojfbutton' title='Использовать карту выхода из тюрьмы.' onclick='useJailCard();' value='Использовать карту' />";
    }

    document.getElementById("nextbutton").title =
      "Бросьте кубик. Если выпадет дубль, вы выйдете из тюрьмы.";

    if (p.jailroll === 0) addAlert("Это первый ход " + p.name + " в тюрьме.");
    else if (p.jailroll === 1)
      addAlert("Это второй ход " + p.name + " в тюрьме.");
    else if (p.jailroll === 2) {
      document.getElementById("landed").innerHTML +=
        "<div>Внимание: если не выпадет дубль, необходимо заплатить штраф $50.</div>";
      addAlert("Это третий ход " + p.name + " в тюрьме.");
    }
...
game_ns._draw_setup.draw_player_wrappers = function (max) {
  ...
  var content_intel = "";
  content_intel +=
    "<select class='player-intel' title='Выберите, управляется ли этот игрок человеком или компьютером.'>";
  content_intel += "	<option value='0' selected='selected'>Человек</option>";
  content_intel += "	<option value='1'>ИИ (Тест)</option>";
  content_intel += "</select>";

  var content_colors = "<select class='player-color' title='Цвет игрока'>";
  for (i = 0; i <= game_ns.available_colors.length - 1; i++) {
    color = game_ns.available_colors[i];
    content_colors +=
      "<option style='color: " + color + ";'>" + color + "</option>";
  }
  content_colors += "</select>";

  for (i = 1; i <= max; i++) {
    content +=
      "<div id='player" + i + "wrap' data-id='" + i + "' class='player-wrap'>";
    content += "Игрок " + i + ": ";
    content +=
      "<input type='text' class='player-name' title='Имя игрока' maxlength='16' value='Игрок " +
      i +
      "' /> ";
...
game_ns._draw_setup.bind_and_invoke_player_color_change = function () {
  $("#player-wrappers .player-color").on(
    "change",
    game_ns._draw_setup.select_on_player_color_change
  );
...
game_ns._draw_setup.bind_and_invoke_players_count_change = function () {
  $("#playernumber").on(
    "change",
    game_ns._draw_setup.select_on_player_number_change
  );
  $("#playernumber").change();
};
...
window.onload = function () {
  ...
  $("<span>").text("Тюрьма").appendTo("#jail");
  ...
  document.getElementById("enlarge40name").innerHTML = "Тюрьма";
  ...
  $("#mortgagebutton").click(function () {
    ...
    if (s.mortgage) {
      if (player[s.owner].money < Math.round(s.price * 0.6)) {
        popup(
          "<p>Вам нужно еще $" +
            (Math.round(s.price * 0.6) - player[s.owner].money) +
            ", чтобы выкупить " +
            s.name +
            ".</p>"
        );
        ...
        popup(
          "<p>" +
            player[s.owner].name +
            ", вы уверены, что хотите выкупить " +
            s.name +
            " за $" +
            Math.round(s.price * 0.6) +
            "?</p>",
          function () {
  ...
  $("#buyhousebutton").on("click", function () {
    ...
    if (s.house < 4 && houseSum >= 32) {
      popup(
        "<p>Все 32 дома заняты. Вы должны подождать, пока один не освободится.</p>"
      );
      return;
    } else if (s.house === 4 && hotelSum >= 12) {
      popup(
        "<p>Все 12 отелей заняты. Вы должны подождать, пока один не освободится.</p>"
      );
      return;
    }
